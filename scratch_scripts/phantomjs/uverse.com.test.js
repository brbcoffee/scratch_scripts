...
var countLoadFinished = 0;
var resourceWait  = 300,
    maxRenderWait = 10000;

page.onResourceRequested = function (req) {
    //console.log('> ' + renderTimeout + '|' + req.id + ' - ' + req.url);
...
page.onResourceReceived = function(res) {
    if (!res.stage || res.stage === 'end') {
        ...
        if (count === 0) {
            renderTimeout = setTimeout(wru(), resourceWait);
        }
    }
};
...
page.onLoadFinished = function(status) {
    countLoadFinished += 1;
    //system.stderr.writeLine('  status: ' + status + ' ' + page.evaluate(function () { return window.location.href; }));
};
...
page.open(
    ...
    function start(status) {
        forcedRenderTimeout = setTimeout( function() {
            //system.stderr.writeLine(" DONE IT " + countLoadFinished);
                    ...
                    wru.test([
                        {
                            name: "it works!",
                            test: function () {
                                // Run the tests in a async block
                                setTimeout(async(function () { // wru.async
                                    // Card Counting
                                    // We check for elemts with 'card' clases in two locations
                                    // Initially they are located inside <div id='initial-tiles'>, the javascript then copies these
                                    // into the <section id="cardSection_0"> element.
                                    wru.assert("Must have at least 8 components", document.getElementsByClassName("component").length >= 3);

                                    // Basic page structure test
                                    wru.assert("Mast footer must exist", document.getElementById("mastfooter"));
                                }), 5);
                            }
                        }
                    ]);
